...
class CalendarBloc {
  ...
  List<Calendar> _calendarList = [
    Calendar(DateTime.now().day, DateTime.now().month, DateTime.now().year,
        DateTime.now().weekday),
    Calendar(DateTime.now().day + 1, DateTime.now().month, DateTime.now().year,
        DateTime.now().weekday)
  ];

  List<Calendar> get calendarList => _calendarList;

  final _calendarListStreamController =
  ...
  //getters
  Stream<List<Calendar>> get calendarListStream =>
      _calendarListStreamController.stream;
  StreamSink<List<Calendar>> get calendarListSink =>
      _calendarListStreamController.sink;

  StreamSink<Calendar> get calendarStartDate =>
      _calendarStartDateStreamController.sink;
  StreamSink<Calendar> get calendarEndDate =>
      _calendarEndDateStreamController.sink;
